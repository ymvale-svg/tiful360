...
>(({ data, taxYear, signatureRef, employerInfo }, ref) => {
  ...
  const fmtDate = (s: string) => s ? new Date(s).toLocaleDateString("en-GB").replaceAll("/", "-") : "";
  const maritalText = ({
    ...
    >
      ...
      <div style={{ marginTop: 12, border: "1px solid #000", padding: 8, fontSize: 10 }}>
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-end", marginTop: 16 }}>
          <div>
            <div style={{ borderTop: "1px solid #000", paddingTop: 2, minWidth: 160 }}>תאריך: {new Date().toLocaleDateString("en-GB").replaceAll("/", "-")}</div>
          </div>
