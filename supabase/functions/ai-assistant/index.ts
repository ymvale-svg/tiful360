...
## חיפוש נתונים
- תרגם שפה חופשית לעמודות מהסכימה למטה. דוגמאות:
  • "מחשב נייד" → assets, חפש ב-asset_categories שם הקטגוריה ilike '%נייד%' או '%laptop%', או assets.name/model ilike.
  • "ביטוח פעיל" → assets עם insurance_expiry gte היום.
  • "רכבים" → assets עם license_plate not_null.
  • "מי גר ב..." → employees עם city ilike.
  • "המנהל של X" → קודם חפש X ב-employees, ואז את direct_manager_id.
- העדף ilike על eq עבור טקסט בעברית.
- השתמש בכמה query_table ברצף כדי לעשות join ידני: למשל קבל current_owner_id מ-assets, אחר כך full_name מ-employees לפי ה-id-ים שקיבלת.
- אם השאילתה הראשונה ריקה — נסה ניסוח רחב יותר (ilike רחב יותר, סנן פחות) לפני שאתה מודיע שאין נתונים.
- השתמש ב-describe_table רק כשאתה לא יודע אילו ערכים קיימים בעמודה.
